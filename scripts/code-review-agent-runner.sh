#!/bin/bash
# Code Review Agent Runner using aider
...
if [ "$REFINEMENT_MODE" = true ]; then
    ...
    AIDER_PROMPT="You are a code review agent. You previously reviewed this code, but the developer has provided feedback requesting changes.

DEVELOPER FEEDBACK:
$REFINEMENT_FEEDBACK

Please redo your code review with the following in mind:
- Address the specific feedback provided above
- Check code style and formatting
- Identify linting violations
- Find logic errors and bugs
- Detect security vulnerabilities
- Identify performance issues
- Check adherence to best practices (DRY, SOLID, etc.)
- Verify error handling completeness
- Check test coverage

Generate a CODE_REVIEW.md file with:
- Severity levels: CRITICAL, HIGH, MEDIUM, LOW, INFO
- File path, line number, issue description, and suggestion for each finding
...
if [ "$REFINEMENT_MODE" = true ]; then
    ...
    "$REPO_ROOT/scripts/vibe-kanban-sync.sh" "$AGENT_NAME" "reviewing" "$ACTUAL_COMMIT"
    echo "Refinement completed: processed commit $REFINEMENT_COMMIT, created new commit $ACTUAL_COMMIT"
else
    # Normal completion - check review status
    if grep -q "CRITICAL\|HIGH" CODE_REVIEW.md 2>/dev/null; then
        "$REPO_ROOT/scripts/vibe-kanban-sync.sh" "$AGENT_NAME" "issues_found" "$ACTUAL_COMMIT"
    else
        "$REPO_ROOT/scripts/vibe-kanban-sync.sh" "$AGENT_NAME" "approved" "$ACTUAL_COMMIT"
    fi
fi
